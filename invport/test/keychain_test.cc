...
{
  // Induce failed read
  const fs::path path = "/tmp/invport/keychain.json";
  fs::remove(path);
  fs::create_directory(path);

  const Keychain key(inv::file::TEMP);
  EXPECT_TRUE(key.KeychainValidity().Failure());
  EXPECT_FALSE(key.Populated());
...
{
  std::ofstream ofstream("/tmp/invport/keychain.json");
  const auto str = inv::json::Json::array();
  ofstream << str;
  ofstream.close();
...
TEST(Key, FilePopulated)
{
  ...
  fs::create_directory("/tmp/invport");
  std::ofstream ofstream("/tmp/invport/keychain.json");
  const auto str = json.dump();
  ofstream << str;
  ofstream.close();

  Keychain key(inv::file::TEMP);
